...
class TestPfamAdapter:
    ...
    def adapter(self, lookup_config):
        """Create PfamAdapter instance."""
        return PfamAdapter(lookup_config)

    def test_adapter_initialization(self, lookup_config):
        """Test PfamAdapter initialization."""
        adapter = PfamAdapter(lookup_config)
        assert adapter.source == KnowledgeSource.PFAM
        assert adapter.config == lookup_config

    def test_get_source(self, adapter):
        """Test get_source returns correct source."""
        assert adapter.get_source() == KnowledgeSource.PFAM
    ...
    def test_get_rate_limit_custom(self):
        ...
        config = LookupConfig(rate_limits={KnowledgeSource.PFAM: 4.0})
    ...
    async def test_search_concepts_success(self, mock_get, adapter):
        ...
        results = await adapter.search_concepts("7tm", limit=10)
        assert isinstance(results, list)
        assert len(results) == 1
        assert results[0].primary_id == "Pfam:PF00001"
        assert results[0].primary_label == "7tm_1"
    ...
    async def test_get_concept_details_success(self, mock_get, adapter):
        ...
        result = await adapter.get_concept_details("PF00001")
        assert result is not None
        assert result.primary_id == "Pfam:PF00001"
    ...
    async def test_get_concept_details_not_found(self, mock_get, adapter):
        ...
        result = await adapter.get_concept_details("PF99999")
        assert result is None

    @pytest.mark.asyncio
    async def test_get_mappings_default(self, adapter):
        """Test get_mappings returns empty list by default."""
        mappings = await adapter.get_mappings("Pfam:PF00001")
