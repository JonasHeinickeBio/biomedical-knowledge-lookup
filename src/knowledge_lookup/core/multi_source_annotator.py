...
class MultiSourceAnnotator:
    ...
    async def annotate_text(
        ...
        # Generate statistics
        stats = self._generate_annotation_stats(source_annotations, consensus_concepts, sources)
    ...
    async def _get_source_annotations(
        ...
        Skips sources whose circuit breaker is open (tracked by
        the ``SourceHealthTracker`` in ``central_lookup``).
        """
        annotations = []
        ...
        for source in sources:
            health = self.central_lookup.health_tracker.get_health(source)
            if health and health.is_open:
                ...
                annotations.append(
                    SourceAnnotation(
                        ...
                        positions=[],
                        processing_time=0.0,
                        error=f"Circuit breaker open ({health.failure_count} failures)",
                    )
                )
            else:
                healthy_sources.append(source)

        # Process healthy sources concurrently where possible
        tasks = []
        ...
        # Process results
        for source, result in zip(healthy_sources, source_results, strict=False):
    ...
    def _generate_annotation_stats(
        ...
        source_annotations: list[SourceAnnotation],
        consensus_concepts: list[ConceptAgreement],
        annotation_sources: list[KnowledgeSource] | None = None,
    ) -> dict[str, Any]:
        """Generate statistics about the annotation process.

        Includes source health snapshot when available.
        """
        ...
        stats = {
            ...
            "source_health": {
                ...
                for src, h in self.central_lookup.health_tracker.all_health().items()
                if annotation_sources is None or src in annotation_sources
            }
            if hasattr(self.central_lookup, "health_tracker")
            else {},
