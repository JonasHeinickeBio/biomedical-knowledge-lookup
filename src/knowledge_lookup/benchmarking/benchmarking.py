...
from ..core import CentralKnowledgeLookup
...
class KnowledgeLookupBenchmarker:
    ...
    def __init__(self, lookup: CentralKnowledgeLookup | None = None):
        self.lookup = lookup or CentralKnowledgeLookup()
        self.metrics: list[BenchmarkMetric] = []

    async def benchmark_single_source(
        self, source: KnowledgeSource, queries: list[str]
    ...
    async def benchmark_combinations(
        self, combinations: list[list[KnowledgeSource]], queries: list[str], parallel: bool = True
    ) -> list[BenchmarkMetric]:
        ...
        for combo in combinations:
            ...
            for query in queries:
                start_time = time.time()
                try:
                    res = await self.lookup.search_concepts(
                        query, sources=combo, parallel=parallel
                    )
                    duration = time.time() - start_time
                    metric = BenchmarkMetric(
                    ...
                    logger.error(
                        f"Benchmark failed for combo {combo_names} with query '{query}': {e}"
                    )

        return results

    async def benchmark_parallel_vs_sequential(
        self, sources: list[KnowledgeSource], queries: list[str]
    ) -> dict[str, list[BenchmarkMetric]]:
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        for name in names:
            ...
            summary["by_name"][name] = {
                ...
                "success_rate": sum(1 for m in name_metrics if m.error_count == 0)
                / len(name_metrics),
            }

        return summary

    def save_results(self, filepath: str | Path):
        """Save benchmark results to a JSON file."""
        data = [asdict(m) for m in self.metrics]
        summary = self.get_summary()

        output = {"summary": summary, "metrics": data}
