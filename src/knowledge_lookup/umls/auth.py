...
class UMLSAuthenticator:
    ...
    def __init__(
        ...
        self.tgt_expires: Optional[float] = None
    ...
    def ensure_tgt(self):
        ...
        if (self.tgt is None or
            self.tgt_expires is None or
            current_time >= self.tgt_expires):
    ...
    def get_service_ticket(self) -> str:
        ...
        if response.status_code != 200:
            raise RuntimeError(f"Service ticket fetch failed: {response.status_code} - {response.text}")
