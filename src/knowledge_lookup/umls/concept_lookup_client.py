...
class UMLSConceptLookupClient:
    ...
    async def get_concepts_for_code(
        self, code: str, source_vocabulary: str
    ) -> list[ConceptMapping]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error getting concepts for code {code} in vocabulary {source_vocabulary}: {e}"
            )
            mappings.append(
                ConceptMapping(
                    code=code, source_vocabulary=source_vocabulary, error_message=str(e)
                )
            )

        return mappings

    async def get_concepts_for_codes(
        self, codes: list[str], source_vocabulary: str
    ) -> ConceptLookupResult:
        """
    ...
    async def get_concepts_from_file(
        self, input_file: str | Path, source_vocabulary: str
    ) -> ConceptLookupResult:
        """
    ...
    async def batch_lookup(
        self, code_vocabulary_pairs: list[tuple[str, str]]
    ) -> dict[str, ConceptLookupResult]:
        """
    ...
    def save_result(
        self, result: ConceptLookupResult, output_file: str | Path, format: str = "txt"
    ) -> None:
        """
    ...
    try:
        async with UMLSConceptLookupClient(
            ...
            result = await client.get_concepts_from_file(
                input_file=args.input, source_vocabulary=args.sabs
            )
