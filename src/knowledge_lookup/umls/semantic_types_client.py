...
class SemanticTypeMapping:
    ...
    semantic_type_uri: Optional[str] = None
...
class UMLSSemanticTypesClient:
    ...
    async def _search_string(self, search_string: str, sabs: Optional[str] = None, 
    ...
    async def _get_semantic_types(self, cui: str) -> Tuple[str, str, str, str]:
        ...
        try:
            params = {}
            response = await self._make_request(
                f"{self.base_uri}{self.content_endpoint}/{cui}", 
                params
            )
    ...
    async def get_semantic_types_for_string(self, search_string: str,
                                          sabs: Optional[str] = None,
                                          search_type: Optional[str] = None) -> List[SemanticTypeMapping]:
        """
        ...
        try:
            ...
            if not cuis:
                mappings.append(SemanticTypeMapping(
    ...
    async def get_semantic_types_for_strings(self, search_strings: List[str],
                                           sabs: Optional[str] = None,
                                           search_type: Optional[str] = None) -> SemanticTypesResult:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                all_mappings.append(SemanticTypeMapping(
    ...
    async def get_semantic_types_from_file(self, input_file: Union[str, Path],
                                         sabs: Optional[str] = None,
                                         search_type: Optional[str] = None) -> SemanticTypesResult:
        """
    ...
    def save_result(self, result: SemanticTypesResult, output_file: Union[str, Path],
                   format: str = "txt") -> None:
        """
    ...
    def _save_excel(self, result: SemanticTypesResult, output_path: Path) -> None:
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            summary_df = pd.DataFrame({
                'Metric': ['Total Processed', 'Successful', 'Failed', 'Success Rate (%)', 'Execution Time (s)'],
                'Value': [result.total_processed, result.successful_mappings, 
