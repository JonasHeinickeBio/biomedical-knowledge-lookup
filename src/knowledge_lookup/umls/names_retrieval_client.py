...
class NameMapping:
    ...
    def is_successful(self) -> bool:
        ...
        return self.error_message is None and (self.name is not None or bool(self.atoms))
...
class UMLSNamesRetrievalClient:
    ...
    async def get_names_for_cui(self, cui: str) -> NameMapping:
        ...
        try:
            ...
            response = await self._make_request(f"{self.base_uri}{content_endpoint}", {})
            ...
            return NameMapping(
                identifier=cui, identifier_type="CUI", cui=cui, name=cui_name, atoms=atoms
            )

        except Exception as e:
            logger.error(f"Error getting names for CUI {cui}: {e}")
            return NameMapping(identifier=cui, identifier_type="CUI", error_message=str(e))
    ...
    async def get_names_for_code(self, code: str, source_vocabulary: str) -> NameMapping:
        ...
        try:
            ...
            logger.error(
                f"Error getting names for code {code} in vocabulary {source_vocabulary}: {e}"
            )
            return NameMapping(
                identifier=code,
                identifier_type="code",
    ...
    async def get_names_for_identifiers(
        self, identifiers: list[str], source_vocabulary: str | None = None
    ) -> NamesResult:
        """
        ...
        for i, result in enumerate(results):
            ...
            elif isinstance(result, NameMapping):
    ...
    async def get_names_from_file(
        self, input_file: str | Path, source_vocabulary: str | None = None
    ) -> NamesResult:
        """
    ...
    async def batch_lookup(
        self, identifier_vocabulary_pairs: list[tuple[str, str | None]]
    ) -> dict[str, NamesResult]:
        """
    ...
    def save_result(
        self, result: NamesResult, output_file: str | Path, format: str = "txt"
    ) -> None:
        """
            ...
            summary_df = pd.DataFrame(
                {
                    ...
                    "Value": [
                        ...
                        result.source_vocabulary or "N/A",
...
async def main():
    ...
    parser.add_argument(
        "-s", "--source", help="Source vocabulary (required for codes, omit for CUIs)"
    )
    parser.add_argument(
