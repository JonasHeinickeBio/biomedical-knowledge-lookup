...
class SearchMapping:
    ...
    def is_successful(self) -> bool:
        ...
        return self.error_message is None and self.results
...
class SearchTermsResult:
    ...
    def success_rate(self) -> float:
        ...
        return (self.successful_searches / self.total_processed * 100) if self.total_processed > 0 else 0.0
...
class UMLSEnhancedSearchClient:
    ...
    async def search_term(self, query_term: str,
                         search_type: Optional[str] = None,
                         vocabularies: Optional[str] = None,
                         ...
                         page_size: int = 25) -> SearchMapping:
        """
    ...
    async def search_terms(self, query_terms: List[str],
                          search_type: Optional[str] = None,
                          vocabularies: Optional[str] = None,
                          input_type: Optional[str] = None,
                          return_id_type: Optional[str] = None,
                          include_obsolete: bool = False,
                          include_suppressible: bool = False,
                          max_results_per_term: Optional[int] = None,
                          page_size: int = 25) -> SearchTermsResult:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                all_mappings.append(SearchMapping(
    ...
    async def search_from_file(self, input_file: Union[str, Path],
                             search_type: Optional[str] = None,
                             vocabularies: Optional[str] = None,
                             input_type: Optional[str] = None,
                             return_id_type: Optional[str] = None,
    ...
    def save_result(self, result: SearchTermsResult, output_file: Union[str, Path],
                   format: str = "txt") -> None:
        """
    ...
    def _save_excel(self, result: SearchTermsResult, output_path: Path) -> None:
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            summary_df = pd.DataFrame({
                'Metric': ['Search Type', 'Vocabularies', 'Total Processed', 'Successful', 'Failed', 'Success Rate (%)', 'Total Results', 'Execution Time (s)'],
                'Value': [result.search_type or 'Default', result.vocabularies or 'All', result.total_processed, 
                         result.successful_searches, result.failed_searches, f"{result.success_rate:.1f}", 
                         result.total_results_found, f"{result.execution_time:.2f}"]
            })
...
async def main():
    ...
    parser.add_argument('-f', '--format', default='txt', choices=['txt', 'json', 'csv', 'excel'], 
                       help='Output format')
    parser.add_argument('--search-type', choices=['exact', 'words', 'normalizedString', 'normalizedWords', 'leftTruncation', 'rightTruncation'],
                       help='Search type')
    parser.add_argument('-s', '--sabs', 
                       help='Comma-separated list of vocabularies')
    parser.add_argument('--input-type', choices=['atom', 'code', 'sourceConcept', 'sourceDescriptor', 'sourceUi'],
                       help='Input type')
    parser.add_argument('--return-id-type', choices=['concept', 'code', 'sourceConcept', 'sourceDescriptor', 'sourceUi', 'aui'],
                       help='Return ID type')
    parser.add_argument('--include-obsolete', action='store_true',
                       help='Include obsolete terms')
    parser.add_argument('--include-suppressible', action='store_true',
                       help='Include suppressible terms')
    parser.add_argument('--max-results-per-term', type=int,
