...
class UMLSStringConceptClient:
    ...
    async def _make_request(self, url: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if session is None:
            raise RuntimeError(
                "Session not initialized. Use async with UMLSStringConceptClient(...)"
            )
    ...
    async def get_concepts_for_string(
        self,
        search_string: str,
        vocabularies: str | None = None,
        term_types: str | None = None,
    ) -> list[StringConceptMapping]:
        """
        ...
        try:
            ...
            mappings.append(
                StringConceptMapping(search_string=search_string, error_message=str(e))
            )

        return mappings

    async def get_concepts_for_strings(
        self,
        search_strings: list[str],
        vocabularies: str | None = None,
        term_types: str | None = None,
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                all_mappings.append(
                    StringConceptMapping(
                        search_string=search_strings[i], error_message=str(result)
                    )
                )
    ...
    async def get_concepts_from_file(
        self,
        input_file: str | Path,
        vocabularies: str | None = None,
        term_types: str | None = None,
    ...
    async def batch_lookup(
        self, string_vocabulary_pairs: list[tuple[str, str, str | None]]
    ) -> dict[str, StringConceptResult]:
        """
        ...
        for config_key, config in config_groups.items():
            result = await self.get_concepts_for_strings(
                config["strings"], config["vocabularies"], config["term_types"]
            )
    ...
    def save_result(
        self, result: StringConceptResult, output_file: str | Path, format: str = "txt"
    ) -> None:
        """
...
async def main():
    ...
    parser.add_argument(
        "-s", "--sabs", help="Comma-separated list of vocabularies (e.g., MSH,SNOMEDCT_US,RXNORM)"
    )
    parser.add_argument("-t", "--ttys", help="Comma-separated list of term types (e.g., PT,SY,IN)")
    parser.add_argument(
    ...
    try:
        async with UMLSStringConceptClient(
            ...
            result = await client.get_concepts_from_file(
                input_file=args.input, vocabularies=args.sabs, term_types=args.ttys
            )
