...
class ZoomaAdapter(KnowledgeSourceAdapter):
    ...
    def _convert_zooma_result_to_concept(self, result: Dict[str, Any]) -> Optional[UnifiedConcept]:
        ...
        try:
            ...
            if 'derivedFrom' in result:
                source = result['derivedFrom'].get('provenance', {}).get('source', {}).get('name', '')
                if source:
