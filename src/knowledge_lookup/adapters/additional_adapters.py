...
class DBpediaAdapter(KnowledgeSourceAdapter):
    ...
    async def get_concept_details(self, concept_id: str) -> Optional[UnifiedConcept]:
        ...
        try:
            ...
            if 'results' in data and 'bindings' in data['results']:
                concept = self._convert_dbpedia_entity_to_unified(concept_id, data['results']['bindings'])
                return concept
    ...
    def _convert_dbpedia_result_to_concept(self, result: Dict[str, Any]) -> Optional[UnifiedConcept]:
    ...
    def _convert_dbpedia_entity_to_unified(self, entity_uri: str, properties: List[Dict[str, Any]]) -> Optional[UnifiedConcept]:
        ...
        try:
            ...
            for prop in properties:
                ...
                if property_uri.endswith('rdfs#label') and prop['value'].get('xml:lang') == 'en':
                    label = value
                elif property_uri.endswith('dbo:abstract') and prop['value'].get('xml:lang') == 'en':
                    abstract = value
            ...
            if abstract:
                concept.definitions.append(abstract[:1000] + "..." if len(abstract) > 1000 else abstract)
            ...
            if '_embedded' in data and 'mappings' in data['_embedded']:
                for mapping in data['_embedded']['mappings']:
                    mappings.append({
                        ...
                        'fromSource': mapping.get('fromTerm', {}).get('datasource', {}).get('name', ''),
                        'toSource': mapping.get('toTerm', {}).get('datasource', {}).get('name', ''),
...
class BioOntologyAdapter(KnowledgeSourceAdapter):
    ...
    def _convert_bioontology_result_to_concept(self, result: Dict[str, Any]) -> Optional[UnifiedConcept]:
