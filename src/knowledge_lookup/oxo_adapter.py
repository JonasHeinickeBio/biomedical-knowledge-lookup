...
class OxOAdapter(KnowledgeSourceAdapter):
    ...
    async def get_concept_by_id(self, concept_id: str, **kwargs) -> Optional[UnifiedConcept]:
        """
    ...
    async def get_concept_details(self, concept_id: str) -> Optional[UnifiedConcept]:
        ...
        """
        return await self.get_concept_by_id(concept_id, distance=3)  # Use distance 3 for detailed search

    async def get_mappings_for_concepts(self,
    ...
    def _parse_search_result(self, result: Dict[str, Any],
                           original_query: Optional[str] = None) -> Optional[UnifiedConcept]:
        """
        ...
        try:
            ...
            for mapping in mappings:
                target_source = self._get_knowledge_source_from_prefix(mapping.get('targetPrefix', ''))
                if target_source:
                    concept.add_mapping(
    ...
    def _get_knowledge_source_from_prefix(self, prefix: str) -> Optional[KnowledgeSource]:
        """
