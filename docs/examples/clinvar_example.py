"""
Example: Using the Clinvar Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.CLINVAR])
    
    print(f"Checking if Clinvar adapter is available...")
    if lookup:
        print(f"  ✓ Clinvar adapter is available")
    else:
        print(f"  ✗ Clinvar adapter is not available")
        return
    
    query = "BRCA1"
    print(f"\\nSearching for '{query}' concepts in Clinvar...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.CLINVAR])
    
    print(f"\\nFound {results.total_found} results from Clinvar:")
