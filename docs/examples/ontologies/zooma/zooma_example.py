"""
Example: Using the Zooma Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.ZOOMA])
    
    print(f"Checking if Zooma adapter is available...")
    if lookup:
        print(f"  ✓ Zooma adapter is available")
    else:
        print(f"  ✗ Zooma adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in Zooma...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.ZOOMA])
    
    print(f"\\nFound {results.total_found} results from Zooma:")
