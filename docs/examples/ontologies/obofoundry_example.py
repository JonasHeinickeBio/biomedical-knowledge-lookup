"""
Example: Using the OBOFoundry Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.OBOFOUNDRY])
    
    print(f"Checking if OBOFoundry adapter is available...")
    if lookup:
        print(f"  ✓ OBOFoundry adapter is available")
    else:
        print(f"  ✗ OBOFoundry adapter is not available")
        return
    
    query = "cancer"
    print(f"
Searching for '{query}' concepts in OBOFoundry...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.OBOFOUNDRY])
    
    print(f"
Found {results.total_found} results from OBOFoundry:")
