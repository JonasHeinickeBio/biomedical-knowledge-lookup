"""
Example: Using the Geneontology Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.GENEONTOLOGY])
    
    print(f"Checking if Geneontology adapter is available...")
    if lookup:
        print(f"  ✓ Geneontology adapter is available")
    else:
        print(f"  ✗ Geneontology adapter is not available")
        return
