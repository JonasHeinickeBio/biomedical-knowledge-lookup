"""
Example: Using the Pubchem Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.PUBCHEM])
    
    print(f"Checking if Pubchem adapter is available...")
    if lookup:
        print(f"  ✓ Pubchem adapter is available")
    else:
        print(f"  ✗ Pubchem adapter is not available")
        return
