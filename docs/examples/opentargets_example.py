"""
Example: Using the Opentargets Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.OPENTARGETS])
    
    print(f"Checking if Opentargets adapter is available...")
    if lookup:
        print(f"  ✓ Opentargets adapter is available")
    else:
        print(f"  ✗ Opentargets adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in Opentargets...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.OPENTARGETS])
    
    print(f"\\nFound {results.total_found} results from Opentargets:")
