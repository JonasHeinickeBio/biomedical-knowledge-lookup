"""
Example: Using the QuickGO Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.QUICKGO])
    
    print(f"Checking if QuickGO adapter is available...")
    if lookup:
        print(f"  ✓ QuickGO adapter is available")
    else:
        print(f"  ✗ QuickGO adapter is not available")
        return
    
    query = "apoptosis"
    print(f"\\nSearching for '{query}' concepts in QuickGO...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.QUICKGO])
    
    print(f"\\nFound {results.total_found} results from QuickGO:")
