"""
Example: Using the PDB Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.PDB])
    
    print(f"Checking if PDB adapter is available...")
    if lookup:
        print(f"  ✓ PDB adapter is available")
    else:
        print(f"  ✗ PDB adapter is not available")
        return
    
    query = "6vxx"
    print(f"\\nSearching for '{query}' concepts in PDB...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.PDB])
    
    print(f"\\nFound {results.total_found} results from PDB:")
