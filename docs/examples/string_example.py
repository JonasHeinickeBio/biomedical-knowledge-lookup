"""
Example: Using the STRING Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.STRING])
    
    print(f"Checking if STRING adapter is available...")
    if lookup:
        print(f"  ✓ STRING adapter is available")
    else:
        print(f"  ✗ STRING adapter is not available")
        return
