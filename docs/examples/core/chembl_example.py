"""
Example: Using the Chembl Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.CHEMBL])
    
    print(f"Checking if Chembl adapter is available...")
    if lookup:
        print(f"  ✓ Chembl adapter is available")
    else:
        print(f"  ✗ Chembl adapter is not available")
        return
    
    query = "cancer"
    print(f"
Searching for '{query}' concepts in Chembl...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.CHEMBL])
    
    print(f"
Found {results.total_found} results from Chembl:")
