"""
Example: Using the Mondo Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.MONDO])
    
    print(f"Checking if Mondo adapter is available...")
    if lookup:
        print(f"  ✓ Mondo adapter is available")
    else:
        print(f"  ✗ Mondo adapter is not available")
        return
    
    query = "cancer"
    print(f"
Searching for '{query}' concepts in Mondo...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.MONDO])
    
    print(f"
Found {results.total_found} results from Mondo:")
