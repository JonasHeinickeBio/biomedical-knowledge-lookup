"""
Example: Using the Ols Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    print(f"Checking if Ols adapter is available...")
    if lookup:
        print(f"  ✓ Ols adapter is available")
    else:
        print(f"  ✗ Ols adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in Ols...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.OLS])
    
    print(f"\\nFound {results.total_found} results from Ols:")
