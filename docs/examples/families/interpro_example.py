"""
Example: Using the InterPro Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.INTERPRO])
    
    print(f"Checking if InterPro adapter is available...")
    if lookup:
        print(f"  ✓ InterPro adapter is available")
    else:
        print(f"  ✗ InterPro adapter is not available")
        return
    
    query = "BRCA1"
    print(f"
Searching for '{query}' concepts in InterPro...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.INTERPRO])
    
    print(f"
Found {results.total_found} results from InterPro:")
