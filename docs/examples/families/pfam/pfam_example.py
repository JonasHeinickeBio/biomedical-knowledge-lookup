"""
Example: Using the Pfam Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.PFAM])
    
    print(f"Checking if Pfam adapter is available...")
    if lookup:
        print(f"  ✓ Pfam adapter is available")
    else:
        print(f"  ✗ Pfam adapter is not available")
        return
    
    query = "BRCA1"
    print(f"\\nSearching for '{query}' concepts in Pfam...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.PFAM])
    
    print(f"\\nFound {results.total_found} results from Pfam:")
