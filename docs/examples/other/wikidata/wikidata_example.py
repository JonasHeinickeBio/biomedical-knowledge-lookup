"""
Example: Using the Wikidata Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.WIKIDATA])
    
    print(f"Checking if Wikidata adapter is available...")
    if lookup:
        print(f"  ✓ Wikidata adapter is available")
    else:
        print(f"  ✗ Wikidata adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in Wikidata...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.WIKIDATA])
    
    print(f"\\nFound {results.total_found} results from Wikidata:")
