"""
Example: Using the BioLinker Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.BIOLINKER])
    
    print(f"Checking if BioLinker adapter is available...")
    if lookup:
        print(f"  ✓ BioLinker adapter is available")
    else:
        print(f"  ✗ BioLinker adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in BioLinker...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.BIOLINKER])
    
    print(f"\\nFound {results.total_found} results from BioLinker:")
