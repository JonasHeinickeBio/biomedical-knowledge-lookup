"""
Example: Using the Tyto Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.TYTO])
    
    print(f"Checking if Tyto adapter is available...")
    if lookup:
        print(f"  ✓ Tyto adapter is available")
    else:
        print(f"  ✗ Tyto adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in Tyto...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.TYTO])
    
    print(f"\\nFound {results.total_found} results from Tyto:")
