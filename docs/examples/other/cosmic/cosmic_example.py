"""
Example: Using the COSMIC Adapter for Biomedical Concept Lookup
"""
...
async def main():
    # Note: COSMIC adapter may require an API key
    # Set COSMIC_API_KEY environment variable
    # os.environ['COSMIC_API_KEY'] = 'your_api_key_here'
    ...
    print(f"Checking if COSMIC adapter is available...")
    if lookup:
        print(f"  ✓ COSMIC adapter is available")
    else:
        print(f"  ✗ COSMIC adapter is not available")
        return
    
    query = "BRAF"
    print(f"\\nSearching for '{query}' concepts in COSMIC...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.COSMIC])
    
    print(f"\\nFound {results.total_found} results from COSMIC:")
