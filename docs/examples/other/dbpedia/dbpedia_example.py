"""
Example: Using the Dbpedia Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.DBPEDIA])
    
    print(f"Checking if Dbpedia adapter is available...")
    if lookup:
        print(f"  ✓ Dbpedia adapter is available")
    else:
        print(f"  ✗ Dbpedia adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in Dbpedia...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.DBPEDIA])
    
    print(f"\\nFound {results.total_found} results from Dbpedia:")
