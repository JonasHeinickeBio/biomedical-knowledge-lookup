"""
Example: Using the Eutils Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.EUTILS])
    
    print(f"Checking if Eutils adapter is available...")
    if lookup:
        print(f"  ✓ Eutils adapter is available")
    else:
        print(f"  ✗ Eutils adapter is not available")
        return
    
    query = "cancer"
    print(f"
Searching for '{query}' concepts in Eutils...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.EUTILS])
    
    print(f"
Found {results.total_found} results from Eutils:")
