"""
Example: Using the KEGG Adapter for Biomedical Concept Lookup
"""
...
async def main():
    ...
    lookup = create_knowledge_lookup(enabled_sources=[KnowledgeSource.KEGG])
    
    print(f"Checking if KEGG adapter is available...")
    if lookup:
        print(f"  ✓ KEGG adapter is available")
    else:
        print(f"  ✗ KEGG adapter is not available")
        return
    
    query = "cancer"
    print(f"\\nSearching for '{query}' concepts in KEGG...")
    results = await lookup.search_concepts(query, sources=[KnowledgeSource.KEGG])
    
    print(f"\\nFound {results.total_found} results from KEGG:")
